...
### Freshness Response Schema

This section defines a second schema used to obtain a live challenge-protected confirmation of attestation freshness.

| Field        | Value                                        |
|--------------|----------------------------------------------|
| Schema       | `bytes32 challenge, uint64 revokedAt`        |
| Schema UID   | TBD                                          |
| Resolver     | `0x0000000000000000000000000000000000000000` |
| Revocable    | `false`                                      |
| EAS Contract | `0xA1207F3BBa224E2c9c3c6D5aF63D0eb1582Ce587` |
...
A Freshness Response is an offchain attestation under this schema:

- `refUID` is set to the UID of the attestation the response concerns.
- `data` is set to RLP encoding of `(bytes32 challenge, uint64 revokedAt)`.
  - `challenge` is the arbitrary value supplied by the verifier issuing the request.
  - `revokedAt` is 0 for valid attestation; for revoked attestations it is the timestamp at which the responding attester revoked its attestation of a descriptor. `revokedAt` is informational only - a verifier MUST NOT compare it against wall-clock time - it is only ever compared against zero.
- `time` and `expirationTime` on a Freshness Response carry no meaning for verification.
...
8. Optionally perform the live freshness challenge and treat its result as the main deciding factor for attestation validity.
...
### Optional Live Freshness Challenge

A verifier MAY, in addition to or instead of relying solely on [Verifying an Attestation](#verifying-an-attestation) step 6, perform a live freshness challenge before treating an attestation as valid:

1. The verifier generates a random 32-byte `challenge`. The verifier MUST use a secure and trusted source of randomness to ensure the `challenge` cannot be replayed.
2. The verifier sends `(attestationUID, challenge)` to a responder. This ERC does not specify a transport protocol used.
3. The responder reads the referenced attestation's current revocation state and signs a Freshness Response with `refUID` set to `attestationUID` and `data` set to `(challenge, revokedAt)`.
4. The verifier checks the response by confirming that the signature is valid, `refUID` equals `attestationUID` and `data.challenge` equals the `challenge` generated in step 1. If all checks pass it treats zero `data.revokedAt` as valid and non-zero `data.revokedAt` as revoked. The exact signature algorithm used by the attester and verifier are out of scope for this ERC.
5. Failure to obtain an appropriate and structurally valid challenge response MUST be treated as a revoked attestation. A wallet SHOULD present the failure information to the user for diagnostics.
...
| Adversarial staleness (untrusted relay) | A verifier with no independent ability to query the canonical EAS contract — for example a signing device relying entirely on an untrusted host for network access — is shown an authentic but stale, pre-revocation view chosen specifically to hide a recent revocation. | [Optional: Live Freshness Challenge](#optional-live-freshness-challenge) lets such a verifier obtain a nonce-bound, live-signed answer instead of trusting whatever view the host relays. |
